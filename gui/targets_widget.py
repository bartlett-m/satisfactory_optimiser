...
from PySide6.QtWidgets import QWidget, QLabel, QFormLayout, QPushButton, QHBoxLayout, QComboBox, QDoubleSpinBox, QSizePolicy
from config_constants import SUPPOSEDLY_UNLIMITED_DOUBLE_SPINBOX_MAX_DECIMALS


class Target():
    # CAUTION: not a real widget - instead a data structure that autogenerates a few widgets
    def __init__(self):
        self.combo_box = QComboBox()
        self._value = QDoubleSpinBox()
        self._value.setRange(0, float("inf"))
        self._value.setDecimals(SUPPOSEDLY_UNLIMITED_DOUBLE_SPINBOX_MAX_DECIMALS)
        self._value.setSizePolicy(QSizePolicy.Policy.MinimumExpanding, QSizePolicy.Policy.Minimum)
